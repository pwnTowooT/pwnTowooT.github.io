...
const githubOwner = (process.env.GITHUB_REPOSITORY_OWNER ?? 'pwnTowooT').toLowerCase()
const repoName =
  process.env.GITHUB_REPOSITORY?.split('/')[1] ?? basename(process.cwd())
const isUserSite = repoName.toLowerCase() === `${githubOwner}.github.io`

export default defineConfig({
  site: `https://${githubOwner}.github.io`,
  base: isUserSite ? undefined : `/${repoName}`,
